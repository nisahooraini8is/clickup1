    ...
    with db.session() as session:
        query = session.query(Emails_data).filter(
            or_(
                ...
                and_(Emails_data.action == action, Emails_data.status != status,Emails_data.status != status1)
            )
...
def candidateprofile(id):
    with db.session() as session:
        ...
        # Querying for additional data matching sender_name, email, or phone
        additional_data = session.query(Emails_data).filter(
            or_(
                and_(Emails_data.sender_name == sender_name,
                     not_(Emails_data.id == id)),  # Exclude initial candidate data
                and_(Emails_data.email == email,
                     not_(Emails_data.id == id)),  # Exclude initial candidate data
                and_(Emails_data.phone_number == phone,
                     not_(Emails_data.id == id))  # Exclude initial candidate data
            )
        ).distinct().all()  # Add distinct() here to remove duplicates
...
def marketing():
    if request.method == 'POST':
        ...
        recipients = ['contact@handshr.com ', 'nhoorain161@gmail.com']
...
def company_detail(id):
    with db.session() as session:
        ...
        additional_data = session.query(Marketing).filter(
            or_(
                and_(Marketing.company == company,
                     not_(Marketing.id == id)),  # Exclude initial candidate data
                and_(Marketing.cperson == person,
                     not_(Marketing.id == id)),  # Exclude initial candidate data
                and_(Marketing.cphone == phone,
                     not_(Marketing.id == id))  # Exclude initial candidate data
            )
        ).distinct().all()  # Add distinct() here to remove duplicates
    ...
    return render_template('company_detail.html', alldata=alldata)
...
def savetarget():
    if request.method == 'POST':
        ...
        print(emails)
        recipients = ['nhoorain161@gmail.com', 'contact@handshr.com']
        recipients2 = emails
